...
; (function ($, window, document, undefined) {
    ...
    ComboTree.prototype.createSourceItemHTML = function (sourceItem) {
      ...
      if (this.options.isMultiple)
        itemHtml += '<span data-id="' + sourceItem.id + '" data-selectable="' + isSelectable + '" class="comboTreeItemTitle ' + selectableClass + '">' + (!selectableLastNode && isSelectable ? '<input type="checkbox" />' : '') + sourceItem.username + '</span>';
      else
        itemHtml += '<span data-id="' + sourceItem.id + '" data-selectable="' + isSelectable + '" class="comboTreeItemTitle ' + selectableClass + '">' + sourceItem.username + '</span>';
    ...
    ComboTree.prototype.isItemInArray = function (item, arr) {
      for (var i = 0; i < arr.length; i++) {
        if (item.id == arr[i].id && item.username == arr[i].username)
          return i + "";
    ...
    ComboTree.prototype.refreshInputVal = function () {
      ...
      if (this.options.isMultiple) {
        if ($(this._elemHoveredItem).parent('li').parent('ul').siblings()[1]) { // Child
          if ($(this._elemHoveredItem).parent('li').parent('ul').parent('li').parent('ul').parent('div').parent('div').siblings('h3')[0].innerHTML == 'Multi Selection With Cascade Option Select') {
            ...
            for (var i = 0; i < this._selectedItems.length; i++) {
              expe += this._selectedItems[i].username;
        ...
        expe = this._selectedItem.username;
      }
      ...
      if (itemId && source) {
        for (let i = 0; i < source.length; i++) {
          if (source[i].id == itemId)
            return { id: source[i].id, username: source[i].username };
    ...
    ComboTree.prototype.getSelectedNames = function () {
      if (this.options.isMultiple && this._selectedItems.length > 0) {
        var tmpArr = [];
        for (i = 0; i < this._selectedItems.length; i++)
          tmpArr.push(this._selectedItems[i].username);
  
        return tmpArr;
      }
      else if (!this.options.isMultiple && this._selectedItem.hasOwnProperty('id')) {
        return this._selectedItem.username;
      }
      return null;
    };
